...
template <int dim>
void Dynamic_Fracture_Problem<dim>::set_runtime_parameters_Dynamic_Slit ()
{
  ...
  density_structure = 2450.0; 

  // Structure parameters
  lame_coefficient_mu = 1.33e10 ; 
  poisson_ratio_nu = 0.2; 
  
  lame_coefficient_lambda = 8.889e+9; //(2 * poisson_ratio_nu * lame_coefficient_mu)/(1.0 - 2 * poisson_ratio_nu);
  ...
  // Timestep size:
  timestep = 1.0e-7; //1.0e-4;
  
  //density to timestep ratio
  density_timestep_ratio = density_structure/timestep;

  // Maximum number of timesteps:
  max_no_timesteps = 10000; //130;
  // changed end_time_value = 1.0e-2;
  end_time_value = 1; // Crack reaches lower left around 1.3e-2 sec
  ...
  timestep_upper_bound = 5.0e-6;
  timestep_lower_bound = 5.0e-7;
  ...
  grid_name  = "mesh_files/dynamic_slit.inp";
  ...
  std::string parent_dir = "./results/dynamic_slit_test";
  create_directory (parent_dir);
  filename_basis  = parent_dir + "/" +  "solution_dynamic_slit_test_"; 
