...
template <int dim>
void Dynamic_Fracture_Problem<dim>::set_runtime_parameters_p_asymmetry (){
  ...
  std::string mesh_file_name = "mesh_files/example3/assymetric_three_point_bending_" + 
  ...
  // Refine the grid:
  global_refinement_steps = 3;
  pred_corr_levels = 1;
  triangulation.refine_global (global_refinement_steps); 

    if (current_test_case.Is_p_asymmetry())
   {
     Point<dim> p1(6.0, 2.75);
     static const SphericalManifold<dim> boundary1(p1);
     triangulation.set_all_manifold_ids_on_boundary(40,40);
     triangulation.set_manifold(40,boundary1);

     Point<dim> p2(6.0, 4.75);
     static const SphericalManifold<dim> boundary2(p2);
     triangulation.set_all_manifold_ids_on_boundary(41,41);
     triangulation.set_manifold(41,boundary2);

     Point<dim> p3(6.0, 6.75);
     static const SphericalManifold<dim> boundary3(p3);
     triangulation.set_all_manifold_ids_on_boundary(42,42);
     triangulation.set_manifold(42,boundary3);
