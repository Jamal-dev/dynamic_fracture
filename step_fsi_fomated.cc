...
template < int dim >
  class BitmapFunction: public Function < dim > {
    public: BitmapFunction(const std::string & filename,
      double x1_, double x2_, double y1_, double y2_, double minvalue_, double maxvalue_): Function < dim > (1),
    f(filename),
    x1(x1_),
    x2(x2_),
    y1(y1_),
...
template < int dim >
  class InitialValuesPressurized: public Function < dim > {
    public: InitialValuesPressurized(const double alpha_eps): Function < dim > (dim + dim + 1) {
...
template < int dim >
  class InitialValuesSneddon3D: public Function < dim > {
    public: InitialValuesSneddon3D(const double alpha_eps): Function < dim > (dim + 1 + dim) {
      _alpha_eps = alpha_eps;
    }
...
template < int dim >
  class InitialValuesHet3D: public Function < dim > {
    public: InitialValuesHet3D(const double alpha_eps): Function < dim > (dim + 1 + dim) {
      _alpha_eps = alpha_eps;
    }
...
template < int dim >
  class InitialValuesMiehe: public Function < dim > {
    public: InitialValuesMiehe(const double alpha_eps,
...
template < int dim >
  class InitialValuesHohlwalzung: public Function < dim > {
    public: InitialValuesHohlwalzung(const double alpha_eps): Function < dim > (dim + 1 + dim) {
      _alpha_eps = alpha_eps;
    }
...
template < int dim >
  class NonhomDirichletBoundaryValues: public Function < dim > {
    public: NonhomDirichletBoundaryValues(const double time,
      std::string test_case,
      const double alpha_eps): Function < dim > (dim + 1 + dim) {
      _time = time;
...
template < int dim >
  void FSI_ALE_Problem < dim > ::set_global_parameters() {
...
// In this method, we set up runtime parameters 
template < int dim >
  void FSI_ALE_Problem < dim > ::set_runtime_parameters_Miehe() {
    ...
    end_time_value = 1.0e-2; // Crack reaches lower left around 1.3e-2 sec
...
// In this method, we set up runtime parameters 
template < int dim >
  void FSI_ALE_Problem < dim > ::set_runtime_parameters_L_shaped() {
...
// In this method, we set up runtime parameters 
template < int dim >
  void FSI_ALE_Problem < dim > ::set_runtime_parameters_Sneddon() {
...
// In this method, we set up runtime parameters 
template < int dim >
  void FSI_ALE_Problem < dim > ::set_runtime_parameters_pressurized() {
...
// In this method, we set up runtime parameters 
template < int dim >
  void FSI_ALE_Problem < dim > ::set_runtime_parameters_screw_domi() {
    // screw
...
template < int dim >
  void FSI_ALE_Problem < dim > ::set_runtime_parameters_Sneddon3D() {
...
template < int dim >
  void FSI_ALE_Problem < dim > ::set_runtime_parameters_Het3D() {
...
template < int dim >
  void FSI_ALE_Problem < dim > ::make_material_vectors() {
  ...
  void FSI_ALE_Problem < dim > ::assemble_system_matrix() {
    ...
    for (; cell != endc; ++cell) {
      ...
      for (unsigned int q = 0; q < n_q_points; ++q) {
        ...
        const Tensor < 2, dim > & stress_term_history = local_quadrature_points_data[q].old_stress;
...
FSI_ALE_Problem < dim > ::assemble_system_rhs() {
  ...
  for (; cell != endc; ++cell) {
    ...
    for (unsigned int q = 0; q < n_q_points; ++q) {
      ...
      const Tensor < 1, dim > grad_pf = ALE_Transformations::get_grad_pf < dim > (q, old_solution_grads);

      const Tensor < 1, dim > old_timestep_grad_pf = ALE_Transformations::get_grad_pf < dim > (q, old_timestep_solution_grads);
...
FSI_ALE_Problem < dim > ::set_initial_bc(const double time) {
  ...
  for (typename std::map < unsigned int, double > ::const_iterator i = boundary_values.begin(); i != boundary_values.end();
    ++i)
...
template < int dim >
  double FSI_ALE_Problem < dim > ::newton_iteration(const double time)

{
  ...
  while (newton_residuum > lower_bound_newton_residuum &&
    ...
    line_search_step = 0;
    for (; line_search_step < max_no_line_search_steps;
      ++line_search_step) {
...
template < int dim >
  void FSI_ALE_Problem < dim > ::newton_iteration_error_based(const double time)
...
template < int dim >
  void
FSI_ALE_Problem < dim > ::project_back_phase_field() {
...
FSI_ALE_Problem < dim > ::output_results(const unsigned int refinement_cycle,
  ...
  std::vector < DataComponentInterpretation::DataComponentInterpretation >
    data_component_interpretation(dim + 1 + dim, DataComponentInterpretation::component_is_scalar);
      ...
      for (unsigned int face = 0; face < GeometryInfo < dim > ::faces_per_cell; ++face)
        if (cell -> face(face) -> at_boundary() &&
          ...
          for (unsigned int q_point = 0; q_point < n_face_q_points; ++q_point) {
            const Tensor < 2, dim > grad_u = ALE_Transformations::get_grad_u < dim > (q_point, face_solution_grads);
...
// to compute the line integrals for stress evaluations
template < int dim >
  double FSI_ALE_Problem < dim > ::goal_functional_stress_x() {
...
template < int dim >
  double
FSI_ALE_Problem < dim > ::compute_cod(
...
template < int dim >
  void
FSI_ALE_Problem < dim > ::compute_functional_values_Sneddon() {
  if (dim == 2) {
...
template < int dim >
  void FSI_ALE_Problem < dim > ::compute_stress_per_cell() {
    ...
    for (unsigned int cell_counter = 0; cell != endc; ++cell, ++cell_counter) {
      ...
      for (unsigned int q = 0; q < n_q_points; ++q) {
        ...
        norm_stress_term += ALE_Transformations::get_deviator_norm < dim > (stress_term);
...
template < int dim >
  void
FSI_ALE_Problem < dim > ::compute_energy() {
...
template < int dim >
  void FSI_ALE_Problem < dim > ::compute_cod_Sneddon3D() {
...
template < int dim >
  bool FSI_ALE_Problem < dim > ::refine_mesh() {
...
template < int dim >
  void FSI_ALE_Problem < dim > ::setup_quadrature_point_history() {
    QGauss < dim > quadrature_formula(degree + 2);
...
template < int dim >
  void FSI_ALE_Problem < dim > ::update_quadrature_point_history() {
    QGauss < dim > quadrature_formula(degree + 2);
...
template < int dim >
  void FSI_ALE_Problem < dim > ::solve_spatial_problem() {
    ...
    do {
      ...
      for (unsigned int j = 0; j < solution_lambda_penal_func.size(); ++j) {
        if ((solution_lambda_penal_func(j) + gamma_penal * (solution(j) - old_timestep_solution(j))) > 0.0)
          solution_lambda_penal_func(j) = solution_lambda_penal_func(j) + gamma_penal * (solution(j) - old_timestep_solution(j));
        else
...
int main() {
  try {
    ...
    FSI_ALE_Problem < dimension > flow_problem(1);
    flow_problem.run();
